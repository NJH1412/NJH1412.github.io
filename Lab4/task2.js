"use strict";

var canvas;
var gl;
var sindex;

var points = [];
var colors = [];
var start = [];

var axis = 0;
var theta = [0, 0, 0];

var thetaLoc;
var u_Translation;
var count = 0;
var index = 0;

function main() {
    canvas = document.getElementById("canvas");

    gl = WebGLUtils.setupWebGL(canvas);
    if (!gl) {
        alert("WebGL isn't available");
    }

    gl.viewport(0, 0, canvas.width, canvas.height);
    gl.clearColor(1.0, 1.0, 1.0, 1.0);

    var select = document.getElementById("select");
    select.addEventListener("click", function () {
        sindex = select.selectedIndex;
        start[] = index;
    });

    canvas.addEventListener("click", function (event) {
        gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
        var rect = canvas.getBoundingClientRect();
        var cx = event.clientX - rect.left;
        var cy = event.clientY - rect.top;
        points.push(vec2(cx, cy));
        points.push(vec2(cx-0.5, cy-0.5*Math.sqrt(3)));
        points.push(vec2(cx+0.5, cy-0.5*Math.sqrt(3)));
        // var t = glMatrix.vec2.fromValues(2 * cx / canvas.width - 1, 2 * (canvas.height - cy) / canvas.height - 1);
        gl.bufferSubData(gl.ARRAY_BUFFER, 8 * index, new Float32Array(points));

    });

    // var triProgram = createProgram(gl, "trivshader", "trifshader");

    var program = initShaders(gl, "trivshader", "trifshader");
    gl.useProgram(program);

    var vBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(points), gl.STATIC_DRAW);

    var vPosition = gl.getAttribLocation(program, "vPosition");
    gl.vertexAttribPointer(vPosition, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vPosition);

    u_Translation = gl.getUniformLocation(program, "u_Translation");


    // var squProgram = createProgram(gl, "squvshader", "squfshader");
    // var rtvProgram = createProgram(gl, "rtvshader", "rtfshader");
    // var rProgram = createProgram(gl, "rvshader", "rfshader");

    // if(!triProgram  || !squProgram  || !rtvProgram || !rProgram ){
    //     console.log("创建程序对象失败");
    //     return;
    // }


    // if(index == 0){
    //     gl.useProgram(triProgram);
    // }

    // if(index == 1){
    //     gl.useProgram(squProgram);
    // }
    //
    // if(index == 2){
    //     console.log(index+"!!");
    //     gl.useProgram(rtvProgram);
    //
    //     makeCube();
    // }
    //
    // if(index == 3){
    //     gl.useProgram(rProgram);
    //
    // }

    renderTriangle();
}


function makeCube() {
    var vertices = [
        glMatrix.vec4.fromValues(-0.5, -0.5, 0.5, 1.0),
        glMatrix.vec4.fromValues(-0.5, 0.5, 0.5, 1.0),
        glMatrix.vec4.fromValues(0.5, 0.5, 0.5, 1.0),
        glMatrix.vec4.fromValues(0.5, -0.5, 0.5, 1.0),
        glMatrix.vec4.fromValues(-0.5, -0.5, -0.5, 1.0),
        glMatrix.vec4.fromValues(-0.5, 0.5, -0.5, 1.0),
        glMatrix.vec4.fromValues(0.5, 0.5, -0.5, 1.0),
        glMatrix.vec4.fromValues(0.5, -0.5, -0.5, 1.0),
    ];

    var vertexColors = [
        glMatrix.vec4.fromValues(0.0, 0.0, 0.0, 1.0),
        glMatrix.vec4.fromValues(1.0, 0.0, 0.0, 1.0),
        glMatrix.vec4.fromValues(1.0, 1.0, 0.0, 1.0),
        glMatrix.vec4.fromValues(0.0, 1.0, 0.0, 1.0),
        glMatrix.vec4.fromValues(0.0, 0.0, 1.0, 1.0),
        glMatrix.vec4.fromValues(1.0, 0.0, 1.0, 1.0),
        glMatrix.vec4.fromValues(0.0, 1.0, 1.0, 1.0),
        glMatrix.vec4.fromValues(1.0, 1.0, 1.0, 1.0)
    ];

    var faces = [
        1, 0, 3, 1, 3, 2, //正
        2, 3, 7, 2, 7, 6, //右
        3, 0, 4, 3, 4, 7, //底
        6, 5, 1, 6, 1, 2, //顶
        4, 5, 6, 4, 6, 7, //背
        5, 4, 0, 5, 0, 1  //左
    ];

    for (var i = 0; i < faces.length; i++) {
        points.push(vertices[faces[i]][0], vertices[faces[i]][1], vertices[faces[i]][2]);

        colors.push(vertexColors[Math.floor(i / 6)][0], vertexColors[Math.floor(i / 6)][1], vertexColors[Math.floor(i / 6)][2], vertexColors[Math.floor(i / 6)][3]);
    }

    thetaLoc = gl.getUniformLocation(program, "theta");
    gl.uniform3fv(thetaLoc, theta);

    gl.enable(gl.DEPTH_TEST);

    // load shaders and initialize attribute buffer

    var cBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, cBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);

    var vColor = gl.getAttribLocation(rtvProgram, "vColor");
    gl.vertexAttribPointer(vColor, 4, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vColor);

    var vBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(points), gl.STATIC_DRAW);

    var vPosition = gl.getAttribLocation(rtvProgram, "vPosition");
    gl.vertexAttribPointer(vPosition, 3, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vPosition);

    render();
}

function renderTriangle() {
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    
    window.requestAnimFrame();

    count+=1;

    var rad = (count % 360) * Math.PI / 180;
    var s = Math.sin(rad) + 1.0;

    gl.uniform4f(u_Translation, s, s, s, 0.0);
    gl.drawArrays(gl.TRIANGLES, 0, 3);

    requestAnimFrame(render);
}